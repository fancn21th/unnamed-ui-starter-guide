...
type MessageContent = 
  ...
  | { type: "thinking"; blocks: ThinkingStepContentBlock[]; title: string; status?: "pending" | "thinking" | "running" | "completed" | "success" | "error"; tasklist?: { dataSource: TodoItem[]; title?: string; status?: "pending" | "confirmed"; editable?: boolean } }
  | { type: "form"; schema: FormSchema }
  | { type: "steps"; steps: Array<ThinkingStepItemProps & { key: React.Key }> }
  | { type: "tasklist"; dataSource: TodoItem[]; title?: string; status?: "pending" | "confirmed"; editable?: boolean }
...
const ConfirmReportPanel = ({ data }: { data: ConfirmPanelData }) => {
  ...
  return (
    ...
    >
      <p className="text-sm text-[var(--text-secondary)]">{state.text}</p>
      <ReportCardList
        cards={displayCards}
...
const mockMessages: MessageData[] = [
  {
    id: "1",
    role: "user",
    content: "数据源我都上传了，缺的东西你去企业知识库“IC - 265”项目里面去找，下面直接帮我生成CSR报告",
        ...
        blocks: [
          {
            type: "text",
            key: "intro",
            content: "我已接收到了用户的材料文件，即将梳理思路 ，为用户输出以“01--3.Chinese_CSR_v2.0_09Dec11 2 -SAMPLE-empty templet”为模板的临床试验报告。为了给用户最严谨的反馈，我需要按照以下步骤执行任务：",
          },
          {
            type: "node",
            key: "tasklist",
                ...
                items: [
                  { content: "查找符合监管要求的模板文件，我将调用企业知识查询工具",
                    ...
                    data: {
                      title: "补充信息",
                      fields: [
                        { name: "title", label: "你关注的时间范围是多久？", type: "radio", options: [
                          { value: "1-2", label: "短期 1-2 年" },
                          { value: "3-5", label: "中期 3-5 年" },
                          { value: "5-10", label: "长期 5-10 年" },
                        ]},
                        { name: "title", label: "你更关注哪些 AI 技术方向？", type: "radio", options: [
                          { value: "llm", label: "大模型" },
                ...
                items: [
                  ...
                  },
                  {
                    ...
                    toolCall: {
                      icon: <Search className="size-4" />,
                      title: "语义匹配",
                      content: "判断是否存在与当前章节高度相似可直接复用的段落，并给出相似度评分",
                    },
                  },
                ],
              },
...
export function MessageList() {
  const defaultAvatarByRole: Record<MessageData["role"], NonNullable<MessageData["avatar"]>> = {
    user: {
  ...
  const updateThinkingTasklist = useCallback(
    (
      messageId: string,
      update: (
        tasklist: NonNullable<Extract<MessageContent, { type: "thinking" }>["tasklist"]>
      ) => NonNullable<Extract<MessageContent, { type: "thinking" }>["tasklist"]>
    ) => {
  ...
  const updateTasklist = useCallback(
    (messageId: string, update: (tasklist: Extract<MessageContent, { type: "tasklist" }>) => Extract<MessageContent, { type: "tasklist" }>) => {
      setMessages((prevMessages) =>
        prevMessages.map((msg) => {
          if (msg.id !== messageId) {
            return msg;
          ...
          if (typeof msg.content !== "object" || msg.content.type !== "tasklist") {
            return msg;
          }
  ...
  const renderThinkingTaskList = (tasklist: NonNullable<Extract<MessageContent, { type: "thinking" }>["tasklist"]>, messageId: string) => (
    <TaskList
  ...
  const renderTaskList = (content: Extract<MessageContent, { type: "tasklist" }>, messageId: string) => (
    <TaskList
  ...
  const renderThinking = (content: Extract<MessageContent, { type: "thinking" }>, messageId: string) => {
    const processedBlocks: ThinkingStepContentBlock[] = content.blocks.map((block) => {
      if (
        block.type === "node" &&
        block.key === "tasklist" &&
        block.node === null &&
  ...
  const renderDocumentCard = (content: Extract<MessageContent, { type: "document-card" }>) => (
    <DocumentCard {...content.card} className="w-[320px]" />
  );

  const renderContent = (content: string | MessageContent | Array<string | MessageContent>, messageId: string): React.ReactNode => {
    if (Array.isArray(content)) {
      return (
        <div className="flex flex-col gap-2">
